...
preload scene= do
      _ <- loadSpritesheet "explosion" (ghRoot <> "explosion.png") {
            frameWidth: 64.0, 
            frameHeight: 64.0, 
            startFrame: 0,
            endFrame: 23,
      ...
      _ <-loadSpritesheet "balls" (ghRoot <> "balls.png") {
            frameWidth: 17.0, 
            frameHeight: 17.0, 
            startFrame: 0,
            endFrame: 4,
...
create scene _state = do
      ...
      _ <- Sprite.add explodeSpriteKey {x:100.0, y: 100.0} scene
               >>= Sprite.playAnimation explodeAnimationKey

      balls <- Sprite.add "balls" {x:100.0, y: 100.0} scene
               >>= Sprite.setFrame 3
      pure unit
